...
local resurrectionSpells = {
	...
	[SL(20484)] = true,  --Druid
--	[SL(115178)] = true, --Monk
	[SL(22999)] = true, --고블린 점퍼 케이블XL
	[SL(8342)] = true, --고블린 점퍼 케이블

}

function InvenRaidFrames3Member_HideResurrection(self, noHideCenterIcon)
	if self.resurrectionBar.ticker then
		--IRF3:Message("Unregister resurrection Timer: ".. tostring(self.resurrectionBar.ticker) )
	...
	if (self.isGhost or self.isDead) and self.unit and UnitHasIncomingResurrection(self.unit) then
		for member in pairs(IRF3.visibleMembers) do
			if UnitExists(member.displayedUnit) then
				...
				local castName, _, _,  castStart, castEnd = UnitCastingInfo(member.displayedUnit)--모든 공격대원의 캐스팅 정보를 얻어온다.
...
--				if not self.resurrectionBar.caster and castName and resurrectionSpells[castName] then -- and (diff > -5 and diff < 5) then--현재 캐스팅 중인 사람이 없고, 부활 주문이 감지되고, 부활 주문이 감지된 시간과 캐스팅 시간의 차이가 5ms 이하일 때 부활 바 표시
					...
					self.resurrectionBar.ticker = self.resurrectionBar.ticker or C_Timer.NewTicker(0.04, function() InvenRaidFrames3Member_OnUpdateResurrectionBar(self) end)
					--IRF3:Message("Register resurrection Timer: ".. tostring(self.resurrectionBar.ticker) )
