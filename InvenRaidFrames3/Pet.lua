...
function InvenRaidFrames3Pet_OnLoad(self)
	...
	tinsert(UnitPopupMenus, self.dropDown:GetName())
	CompactUnitFrame_SetMenuFunc(self, CompactUnitFrameDropDown_Initialize)
	self:RegisterEvent("UNIT_FLAGS") 
end

local function setPetWatch(self)
	local inCombat = InCombatLockdown() or UnitAffectingCombat("player")
	if self and self.unit and not inCombat then
		...
		self.petRegister = false
	else
		self.petRegister = true
	end
end
...
function InvenRaidFrames3Pet_OnShow(self)
	...
	setPetWatch(self)	--RegisterUnitWatch(self)
...
--			self:RegisterEvent("UNIT_ABSORB_AMOUNT_CHANGED")
		...
		if not self.ticker then
			self.ticker = C_Timer.NewTicker(1, function() InvenRaidFrames3Member_OnUpdate(self) end)
		end
...
eventHandler.UNIT_POWER_UPDATE = InvenRaidFrames3Member_UpdatePower
...
function IRF3:UpdatePetGroup() 		
	local inCombat = InCombatLockdown() or UnitAffectingCombat("player")
	if IRF3.db.usePet == 2 and not inCombat then
