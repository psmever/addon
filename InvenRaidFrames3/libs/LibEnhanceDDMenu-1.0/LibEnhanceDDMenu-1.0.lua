local MAJOR_VERSION, MINOR_VERSION = "LibEnhanceDDMenu-1.0", 6
...
local blizzardDropdowns = {
	...
	"^TargetFrameDropDown$",
	"^FocusFrameDropDown$",
	"^Boss(.+)TargetFrameDropDown$",
	"^PartyMemberFrame(%d+)DropDown$",
	"^FriendsDropDown$",
	"^CompactRaidGroupMember(%d+)DropDown$",
...
function lib:DisableUnitMenu(menu)
	if UnitPopupMenus[menu] then --or UnitPopupButtons["LARGE_FOCUS"] then
