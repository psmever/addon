...
local classList = { "WARRIOR", "PRIEST", "ROGUE", "MAGE", "WARLOCK", "HUNTER", "DRUID", "SHAMAN", "PALADIN" }
local powerColor = { WARRIOR = "1", PRIEST = "05", ROGUE = "3", MAGE = "0", WARLOCK = "0", HUNTER = "2", DRUID = "013", SHAMAN = "09", PALADIN = "0" }
...
local function createPreview()
	...
	preview.border = CreateFrame("Frame", nil, preview, BackdropTemplateMixin and "BackdropTemplate")
	...
	for i = 1, 8 do
		...
		for j = 1, 5 do
			...
			preview.headers[i].members[j].class = classList[random(1, table.getn(classList))]
...
---
	...
	preview.petHeader.border = CreateFrame("Frame", nil, preview.petHeader, BackdropTemplateMixin and "BackdropTemplate")
	...
	for i = 1, 40 do
		...
		preview.petHeader.members[i].name:SetPoint("CENTER", preview.petHeader.members[i].healthBar, 0, 0)
		...
		for i = 1, 8 do
			if checkHeader(numMembers[show], i) then
				...
				if IRF3.db.groupby == "GROUP" and IRF3.db.partyTag then
					...
					preview.headers[i].partyTag.tex:SetColorTexture(IRF3.db.partyTagRaid[1], IRF3.db.partyTagRaid[2], IRF3.db.partyTagRaid[3])
