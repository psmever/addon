...
--test
	menu.enableClickCast = LBO:CreateWidget("CheckBox", parent, L["클릭 캐스팅 사용"], L["클릭 캐스팅 사용desc"], nil, nil, true,
...
--		end
		for i = 1, GetNumTalentTabs() do
			addOverrideSpells(IRF3.overrideClickCastingSpells[GetNumTalentTabs(i)])
		end
	end
	...
	local function dropDownInitialize(self, level)
		...
		if level then
			...
			if level == 1 then
				...
				local cnt = hasSpellTab(1) + hasSpellTab(2) + hasSpellTab(3) + hasSpellTab(4)
				if cnt > 0 then
				...
				--if self.button ~= 1 and self.button ~= 2 then
					...
					info.text, info.arg1, info.arg2 = NONE, NONE
					info.checked = self.arg1 == info.arg1 and self.arg2 == info.arg2
					UIDropDownMenu_AddButton(info, level)
				--end
			elseif level == 2 then
				if type(UIDROPDOWNMENU_MENU_VALUE) == "number" then
					...
					name, _, start, last = GetSpellTabInfo(3)
					if name then
						for i = start + 1, start + last do
							info.text = getSpellName(GetSpellBookItemName(i, BOOKTYPE_SPELL))
							if checkSpell(info.text, i, BOOKTYPE_SPELL) then
					...
					name, _, start, last = GetSpellTabInfo(4)
					if name then
						for i = start + 1, start + last do
							info.text = getSpellName(GetSpellBookItemName(i, BOOKTYPE_SPELL))
							if checkSpell(info.text, i, BOOKTYPE_SPELL) then
	...
	menu.clickKeys = CreateFrame("Frame", nil, parent, BackdropTemplateMixin and "BackdropTemplate")
...
--	menu.clickKeys:SetPoint("TOPLEFT", menu.enableClickCast, "BOTTOMRIGHT", 0, -10)
	...
	local function getKeyValue(modifilter)
		...
		elseif IRF3.ccdb[modifilter] == NONE then
			modifilter = L["없음"]
